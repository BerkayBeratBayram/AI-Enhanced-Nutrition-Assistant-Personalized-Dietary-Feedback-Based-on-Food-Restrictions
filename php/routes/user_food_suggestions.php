...
$api_url = "http://127.0.0.1:8000/recommendation_disease_based/?user_id=" . urlencode(string: $user_id) . "&limit=$limit&offset=$offset";
...
$collab_url = "http://127.0.0.1:8000/user_collaborative_filtering/$user_id";
$ch_collab = curl_init($collab_url);
curl_setopt($ch_collab, CURLOPT_RETURNTRANSFER, true);
$collab_response = curl_exec($ch_collab);
$collab_http_code = curl_getinfo($ch_collab, CURLINFO_HTTP_CODE);
curl_close($ch_collab);

$collab_recipes = [];
if ($collab_http_code === 200) {
    $collab_recipes = json_decode($collab_response, true);
}
...
<div class="content-wrapper">
    <div class="container-fluid">
        <div class="content-header">
            <div class="row mb-2">
                <div class="col-sm-6">
                   <h1 class="m-0">Other Users Recipes Suitable for <?= htmlspecialchars(ucfirst($disease)); ?></h1>

                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="home.php">Home</a></li>
                        <li class="breadcrumb-item active">Dishes Recipes</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <?php if (!empty($recipes)): ?>
        <?php foreach ($recipes as $recipe): ?>
            <tr>
                ...
                <td>
                    <button class="btn btn-info btn-sm" onclick="showRecipeDetails(<?= $recipe['recipe_id']; ?>)">Details</button>
...
<script>
    console.log("RECIPE JSON:", <?= json_encode($recipes); ?>);
</script>
